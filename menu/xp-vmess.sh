#!/bin/bash
# Script to check and remove expired VMess accounts and log them

# Configuration Variables
database="/usr/local/etc/v2ray/database.db"
json="/usr/local/etc/v2ray/config.json"
log_file="/usr/local/etc/v2ray/vmess.log"
...
expired_users=$(sqlite3 "$database" "SELECT email, uuid, expired FROM vmess WHERE expired <= '$today';")
...
while IFS='|' read -r user uuid exp_date; do
    if [[ -n "$user" && -n "$uuid" ]]; then
        ...
        sqlite3 "$database" "DELETE FROM vmess WHERE email='$user';"
    fi
done <<< "$expired_users"
